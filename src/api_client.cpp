...
void configure(const String &host, uint16_t port, bool tls) {
    s_host = host; s_port = port; s_tls = tls;
}
void setDeviceToken(const String &token) { s_token = token; }
...
RegisterResult registerDevice(const String &deviceId, const String &name) {
    ...
    if (!beginRequest(http, API_PATH_REGISTER)) {
        out.error = "begin failed"; return out;
    }
    http.addHeader("Content-Type", "application/json");
    JsonDocument req;
    req["device_id"] = deviceId;
    if (name.length()) req["name"] = name;
    ...
    JsonDocument doc;
    if (deserializeJson(doc, resp)) { out.error = "JSON parse failed"; return out; }
    out.ok = true;
...
MeResult getMe() {
    ...
    HTTPClient http;
    if (!beginRequest(http, API_PATH_ME)) { out.error = "begin failed"; return out; }
    http.addHeader("Authorization", "Bearer " + s_token);
    int code = http.GET();
    String resp = http.getString();
    http.end();
    if (code != 200) { out.error = String("HTTP ") + code; return out; }
    JsonDocument doc;
    if (deserializeJson(doc, resp)) { out.error = "JSON parse failed"; return out; }
    out.ok = true;
    out.paired = doc["paired"] | false;
...
bool setPersona(int personaId, String *errorOut) {
    ...
    if (!beginRequest(http, API_PATH_SET_PERSONA)) {
        if (errorOut) *errorOut = "begin failed"; return false;
    }
    http.addHeader("Authorization", "Bearer " + s_token);
    http.addHeader("Content-Type", "application/json");
    JsonDocument req; req["persona_id"] = personaId;
    String body; serializeJson(req, body);
    int code = http.POST(body);
    String resp = http.getString();
    http.end();
    if (code != 200) {
        if (errorOut) *errorOut = String("HTTP ") + code + ": " + resp;
        return false;
    }
    return true;
}
