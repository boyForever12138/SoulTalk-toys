...
void WebSocketsClient::begin(IPAddress host, uint16_t port, const char *url,
                             const char *protocol) {
...
void WebSocketsClient::beginSSL(const char *host, uint16_t port,
                                const char *url, const char *fingerprint,
                                const char *protocol) {
...
void WebSocketsClient::beginSSL(String host, uint16_t port, String url,
                                String fingerprint, String protocol) {
  beginSSL(host.c_str(), port, url.c_str(), fingerprint.c_str(),
           protocol.c_str());
}

void WebSocketsClient::beginSslWithCA(const char *host, uint16_t port,
...
void WebSocketsClient::beginSslWithClientKey(const char *host, uint16_t port,
                                             const char *url,
                                             const char *CA_cert,
...
void WebSocketsClient::beginSslWithBundle(const char *host, uint16_t port,
                                          const char *url,
...
#else
void WebSocketsClient::beginSSL(const char *host, uint16_t port,
                                const char *url, const uint8_t *fingerprint,
                                const char *protocol) {
  begin(host, port, url, protocol);
  _client.isSSL = true;
  _fingerprint = fingerprint;
...
void WebSocketsClient::setSSLClientCertKey(const char *clientCert,
                                           const char *clientPrivateKey) {
  setSSLClientCertKey(new BearSSL::X509List(clientCert),
                      new BearSSL::PrivateKey(clientPrivateKey));
}

void WebSocketsClient::setSSLClientCertKey(
...
void WebSocketsClient::beginSocketIO(const char *host, uint16_t port,
                                     const char *url, const char *protocol) {
  begin(host, port, url, protocol);
  _client.isSocketIO = true;
}

void WebSocketsClient::beginSocketIO(String host, uint16_t port, String url,
                                     String protocol) {
  beginSocketIO(host.c_str(), port, url.c_str(), protocol.c_str());
}

#if defined(HAS_SSL)
void WebSocketsClient::beginSocketIOSSL(const char *host, uint16_t port,
                                        const char *url, const char *protocol) {
  begin(host, port, url, protocol);
  _client.isSocketIO = true;
  _client.isSSL = true;
...
void WebSocketsClient::beginSocketIOSSL(String host, uint16_t port, String url,
                                        String protocol) {
  beginSocketIOSSL(host.c_str(), port, url.c_str(), protocol.c_str());
}

#if defined(SSL_BARESSL)
void WebSocketsClient::beginSocketIOSSLWithCA(const char *host, uint16_t port,
                                              const char *url,
                                              BearSSL::X509List *CA_cert,
                                              const char *protocol) {
  begin(host, port, url, protocol);
...
void WebSocketsClient::loop(void) {
  ...
  if (!clientIsConnected(&_client)) {
    ...
    (WEBSOCKETS_NETWORK_TYPE == NETWORK_SAMD_SEED) || \
    (WEBSOCKETS_NETWORK_TYPE == NETWORK_UNOWIFIR4)
      ...
      if (_CA_cert) {
        ...
        if (_client_cert && _client_key) {
          DEBUG_WEBSOCKETS(
              "[WS-Client] setting client certificate and private key\n");
          _client.ssl->setCertificate(_client_cert);
          _client.ssl->setPrivateKey(_client_key);
        } else {
          DEBUG_WEBSOCKETS(
              "[WS-Client] no client certificate and/or private key set\n");
        }
...
#if defined(HAS_SSL)
    if (_client.isSSL && _connectIp.length() > 0) {
      // Connect to specific IP address, but use _host for SNI + Host header.
      // Required when the domain points to a CDN that does not pass through
      // WebSocket Upgrade headers, but the origin server is reachable by IP.
      IPAddress ip;
      if (ip.fromString(_connectIp.c_str())) {
        DEBUG_WEBSOCKETS("[WS-Client] connect to %s:%u (SNI=%s)\n",
                         _connectIp.c_str(), _port, _host.c_str());
        _ws_connected =
            _client.ssl->connect(ip, _port, _host.c_str(), NULL, NULL, NULL);
      }
    } else {
      _ws_connected =
          _client.tcp->connect(_host.c_str(), _port, WEBSOCKETS_TCP_TIMEOUT);
    }
#else
    _ws_connected =
        _client.tcp->connect(_host.c_str(), _port, WEBSOCKETS_TCP_TIMEOUT);
 ...
 * @return true if ok
 */
bool WebSocketsClient::sendTXT(uint8_t *payload, size_t length,
...
bool WebSocketsClient::sendTXT(char *payload, size_t length,
                               bool headerToPayload) {
 ...
 * @return true if ok
 */
bool WebSocketsClient::sendBIN(uint8_t *payload, size_t length,
                               bool headerToPayload) {
  if (clientIsConnected(&_client)) {
    return sendFrame(&_client, WSop_binary, payload, length, true,
                     headerToPayload);
  }
  return false;
}
 ...
 */
void WebSocketsClient::setAuthorization(const char *user,
                                        const char *password) {
  if (user && password) {
    String auth = user;
    auth += ":";
    auth += password;
    _client.base64Authorization =
        base64_encode((uint8_t *)auth.c_str(), auth.length());
  }
}
...
/**
 * Override TCP connection target IP while keeping _host for SNI/Host header.
 * Useful when the domain DNS-resolves to a CDN that does not support
 * WebSockets, but the origin server is reachable directly by IP. Pass empty
 * string or NULL to disable.
 */
void WebSocketsClient::setConnectIP(const char *ip) {
  _connectIp = ip ? ip : "";
}
...
#if defined(HAS_SSL)
  String protocol = (_client.isSSL) ? WEBSOCKETS_STRING("wss://")
                                    : WEBSOCKETS_STRING("ws://");
#else
...
void WebSocketsClient::handleClientData(void) {
  if ((_client.status == WSC_HEADER || _client.status == WSC_BODY) &&
    ...
    DEBUG_WEBSOCKETS(
        "[WS-Client][handleClientData] header response timeout.. "
        "disconnecting!\n");
...
void WebSocketsClient::sendHeader(WSclient_t *client) {
  ...
  DEBUG_WEBSOCKETS("[WS-Client][sendHeader] handshake %s",
                   (uint8_t *)handshake.c_str());
  write(client, (uint8_t *)handshake.c_str(), handshake.length());

#if (WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266_ASYNC)
  client->tcp->readStringUntil('\n', &(client->cHttpLine),
                               std::bind(&WebSocketsClient::handleHeader, this,
                                         client, &(client->cHttpLine)));
#endif
...
void WebSocketsClient::handleHeader(WSclient_t *client, String *headerLine) {
  ...
  if (headerLine->length() > 0 && client->isSocketIO &&
      client->status == WSC_BODY && client->cSessionId.length() == 0) {
  ...
  if (headerLine->length() > 0) {
      ...
      if (headerName.equalsIgnoreCase(WEBSOCKETS_STRING("Connection"))) {
        ...
        if (headerValue.equalsIgnoreCase(WEBSOCKETS_STRING("websocket"))) {
          client->cIsWebsocket = true;
                     ...
                     WEBSOCKETS_STRING("Sec-WebSocket-Accept"))) {
        client->cAccept = headerValue;
        client->cAccept.trim();  // see rfc6455
      } else if (headerName.equalsIgnoreCase(
                     WEBSOCKETS_STRING("Sec-WebSocket-Protocol"))) {
        client->cProtocol = headerValue;
      } else if (headerName.equalsIgnoreCase(
                     WEBSOCKETS_STRING("Sec-WebSocket-Extensions"))) {
        client->cExtensions = headerValue;
      } else if (headerName.equalsIgnoreCase(
                     WEBSOCKETS_STRING("Sec-WebSocket-Version"))) {
        client->cVersion = headerValue.toInt();
      } else if (headerName.equalsIgnoreCase(WEBSOCKETS_STRING("Set-Cookie")) &&
                 headerValue.indexOf(" io=") > -1) {
        if (headerValue.indexOf(';') > -1) {
          client->cSessionId = headerValue.substring(
...
#if (WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266_ASYNC)
    client->tcp->readStringUntil('\n', &(client->cHttpLine),
                                 std::bind(&WebSocketsClient::handleHeader,
    ...
    if (client->isSocketIO && client->cSessionId.length() == 0 &&
        clientIsConnected(client)) {
      DEBUG_WEBSOCKETS(
          "[WS-Client][handleHeader] still missing cSessionId try socket.io "
          "V3\n");
    ...
    if (ok) {
      switch (client->cCode) {
        ...
        default:   ///< Server dont unterstand requrst
          ok = false;
          DEBUG_WEBSOCKETS(
              "[WS-Client][handleHeader] serverCode is not 101 (%d)\n",
              client->cCode);
    ...
    if (ok) {
      ...
      runCbEvent(WStype_CONNECTED, (uint8_t *)client->cUrl.c_str(),
                 client->cUrl.length());
#if (WEBSOCKETS_NETWORK_TYPE != NETWORK_ESP8266_ASYNC)
      ...
      if (client->cSessionId.length() > 0) {
        ...
        if (clientIsConnected(client) && _client.tcp->available()) {
          ...
          DEBUG_WEBSOCKETS(
              "[WS-Client][handleHeader] still data in buffer (%d), clean "
              "up.\n",
 ...
 * @param pongTimeout uint32_t millis after which pong should timout if not
 * received
 * @param disconnectTimeoutCount uint8_t how many timeouts before disconnect,
 * 0=> do not disconnect
 */
void WebSocketsClient::enableHeartbeat(uint32_t pingInterval,
                                       uint32_t pongTimeout,
                                       uint8_t disconnectTimeoutCount) {
  WebSockets::enableHeartbeat(&_client, pingInterval, pongTimeout,
